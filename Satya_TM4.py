...
with open('modular.csv', 'rb') as ex:
    ...
    d = [i for i in d if i]
T = 30     # No of periods = decision epochs T periods, T epochs. closing epoch not included
epochs = range(T)
for t in epochs:
...
shftcost = [[0,4],[6,0]]    # shftcost[i][j] shifting cost from station i to station j
sal = [13,12]    # Cost retrieved upon uninstalling a module without shifting
...
for t in epochs:
    for s in ms:
        y[t,s] = tm4.addVar(vtype = GRB.INTEGER, lb =0, name = 'y'+str(t+1)+"_"+str(s+1))
ys = {} # For # of modules shifted from i to j station at epoch t : yst_ij
for t in epochs:
    for i in ms:
        for j in ms:
            ys[t,i,j] = tm4.addVar(vtype = GRB.INTEGER, lb =0, name = 'ys'+str(t+1) +"_"+str(i+1)+str(j+1))
x = {}           # demand flow from station s to retailer r at epoch t
for t in epochs:
    for s in ms:
        for r in rt:
            x[t,s,r] = tm4.addVar(vtype = GRB.INTEGER, lb =0, name = 'x'+str(t+1)+"_"+str(s+1)+str(r+1))
ad = {}  # multiple of module capacity to be additionally deployed to station i at epoch t or removed
for t in epochs:
    for s in ms:
        ad[t,s] = tm4.addVar(vtype = GRB.INTEGER, lb = -10, name = 'ad'+str(t+1)+"_"+str(s+1)) # set lb accly. 
lnd = {} # Binary variable to know if station s is being used in period t
for t in epochs:
    for s in ms:
        lnd[t,s] = tm4.addVar(vtype = GRB.BINARY, name = 'lnd'+str(t+1)+"_"+str(s+1))
lost = {} # Lost sales at cusstomer r in period t - no of units 
for t in epochs:
    for r in rt:
        lost[t,r] = tm4.addVar(vtype = GRB.INTEGER, lb =0, name = 'lost'+str(t+1)+"_"+str(r+1))
u = {}   # unused capacity at station s in period t
for t in epochs:
    for s in ms:
        u[t,s] = tm4.addVar(vtype = GRB.INTEGER, lb =0, ub = cap-1, name = 'u'+str(t+1)+"_"+str(s+1))
tm4.update()
...
for t in epochs:
    for r in rt:      # 1) for each retailer in each period demand fulfillment
        tm4.addConstr(lost[t,r]+quicksum(x[t,s,r] for s in ms) == d[t][r],"c%d" % k)
        k+=1
for t in epochs:
    for s in ms:      # 2) for each station in each period capacity distribution
        tm4.addConstr(u[t,s] + quicksum(x[t,s,r] for r in rt) == cap*y[t,s],"c%d" % k)
        k+=1
        tm4.addConstr(ys[t,s,s]== 0, "c%d" % k)
        k+=1
        tm4.addConstr(y[t,s] <= ((quicksum(d[t][r] for r in rt)/cap) + 1)*lnd[t,s], "c%d" % k) # 3) for each station's land use
        k+=1
        tm4.addConstr(y[t,s] >= lnd[t,s], "c%d" % k)
        k+=1
for s in ms:
    tm4.addConstr(ad[0,s]== 0, "c%d" % k)
    tm4.addConstr(y[1,s]==y[0,s]-quicksum(ys[0,s,i] for i in ms)+ad[1,s],"c%d" % k) # 4) Dyn Eq for the first period
    k+=1
    for t in range(1,T-1):     # for each station SYSTEM DYNAMICS EQN  includes [1, 2, .. ,T-1]
        tm4.addConstr(y[t+1,s]==y[t,s]-quicksum(ys[t,s,i] for i in ms) + quicksum(ys[t-1,i,s] for i in ms)+ad[t+1,s],"c%d" % k)
        k+=1
    tm4.addConstr(y[T-1,s]==y[T-2,s]+quicksum(ys[T-3,i,s] for i in ms),"c%d" % k)   # Dyn Eq for the last period
    k+=1
...
for t in epochs:
    for s in ms:
        if ad[t,s]>0:
            cost2a+=setup[s]*ad[t,s]         # Additional setup cost or salvage from retired modules
        else:
            cost2b+=sal[s]*ad[t,s]
cost2 = cost2a+cost2b
...
tm4.setObjective(cost1+cost2+cost3+cost4+cost5+cost6+cost7, GRB.MINIMIZE)
#OPTIMIZING______________________________________________________________
tm4.optimize()
tm4.write("tm4_LP.lp")
sol = open('tm4_soln.txt','w+')
for v in tm4.getVars():
    print(v.varName, v.x , file=sol)
print ( "Obj: ", tm4.objVal, file=sol)
